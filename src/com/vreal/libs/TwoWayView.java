...
//		initializeScrollbars(a,context);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (index < 0) {
				Log.e(LOGTAG, "onInterceptTouchEvent could not find pointer with id " + mActivePointerId + " - did TwoWayView receive an inconsistent " + "event stream?");
				return false;
			}
		...
		case MotionEvent.ACTION_UP: {
			...
			case TOUCH_MODE_DRAGGING:
				...
				if (Math.abs(velocity) >= mFlingVelocity) {
					...
					mScroller.fling(0, 0, (int) (mIsVertical ? 0 : velocity), (int) (mIsVertical ? velocity : 0), (mIsVertical ? 0 : Integer.MIN_VALUE), (mIsVertical ? 0 : Integer.MAX_VALUE),
	...
	private ArrowScrollFocusResult arrowScrollFocused(final int direction) {
		...
		if (newFocus != null) {
			...
			if (mSelectedPosition != INVALID_POSITION && positionOfNewFocus != mSelectedPosition) {
				...
				if (selectablePosition != INVALID_POSITION && ((movingForward && selectablePosition < positionOfNewFocus) || (movingBackward && selectablePosition > positionOfNewFocus))) {
					return null;
				}
			}
	...
	private boolean arrowScrollImpl(int direction) {
		...
		if (amountToScroll > 0) {
			scrollListItemsBy(direction == View.FOCUS_UP || direction == View.FOCUS_LEFT ? amountToScroll : -amountToScroll);
			needToRedraw = true;
		}
	...
	private int amountToScroll(int direction, int nextSelectedPosition) {
		...
		if (direction == View.FOCUS_DOWN || direction == View.FOCUS_RIGHT) {
			...
			final int viewToMakeVisibleStart = (mIsVertical ? viewToMakeVisible.getTop() : viewToMakeVisible.getLeft());
			final int viewToMakeVisibleEnd = (mIsVertical ? viewToMakeVisible.getBottom() : viewToMakeVisible.getRight());
			...
			if (nextSelectedPosition != INVALID_POSITION && (goalEnd - viewToMakeVisibleStart) >= getMaxScrollAmount()) {
			...
			if (nextSelectedPosition != INVALID_POSITION && (viewToMakeVisibleEnd - goalStart) >= getMaxScrollAmount()) {
				// Item already has enough of it visible, changing selection is
				// good enough
				return 0;
			}
	...
	private boolean handleKeyEvent(int keyCode, int count, KeyEvent event) {
		...
		if (action != KeyEvent.ACTION_UP) {
			...
			case KeyEvent.KEYCODE_SPACE:
				if (KeyEventCompat.hasNoModifiers(event)) {
					handled = resurrectSelectionIfNeeded() || pageScroll(mIsVertical ? View.FOCUS_DOWN : View.FOCUS_RIGHT);
				} else if (KeyEventCompat.hasModifiers(event, KeyEvent.META_SHIFT_ON)) {
					handled = resurrectSelectionIfNeeded() || fullScroll(mIsVertical ? View.FOCUS_UP : View.FOCUS_LEFT);
				}
			...
			case KeyEvent.KEYCODE_PAGE_UP:
				if (KeyEventCompat.hasNoModifiers(event)) {
					handled = resurrectSelectionIfNeeded() || pageScroll(mIsVertical ? View.FOCUS_UP : View.FOCUS_LEFT);
				} else if (KeyEventCompat.hasModifiers(event, KeyEvent.META_ALT_ON)) {
					handled = resurrectSelectionIfNeeded() || fullScroll(mIsVertical ? View.FOCUS_UP : View.FOCUS_LEFT);
				}
				break;
			...
			case KeyEvent.KEYCODE_PAGE_DOWN:
				if (KeyEventCompat.hasNoModifiers(event)) {
					...
					handled = resurrectSelectionIfNeeded() || fullScroll(mIsVertical ? View.FOCUS_DOWN : View.FOCUS_RIGHT);
				}
				break;
	...
	private void updateOverScrollState(int delta, int overscroll) {
		overScrollByInternal((mIsVertical ? 0 : overscroll), (mIsVertical ? overscroll : 0), (mIsVertical ? 0 : mOverScroll), (mIsVertical ? mOverScroll : 0), 0, 0, (mIsVertical ? 0
				: mOverscrollDistance), (mIsVertical ? mOverscrollDistance : 0), true);
	...
	private boolean scrollListItemsBy(int incrementalDelta) {
		...
		final boolean cannotScrollDown = (firstPosition == 0 && firstStart >= paddingStart && incrementalDelta >= 0);
		final boolean cannotScrollUp = (firstPosition + childCount == mItemCount && lastEnd <= end && incrementalDelta <= 0);
	...
	private void layoutChildren() {
		...
		try {
			...
			if (mItemCount == 0) {
				...
				throw new IllegalStateException("The content of the adapter has changed but " + "TwoWayView did not receive a notification. Make sure the content of "
						+ "your adapter is not modified from a background thread, but only " + "from the UI thread. [in TwoWayView(" + getId() + ", " + getClass() + ") with Adapter("
						+ mAdapter.getClass() + ")]");
			}
			...
			if (selected != null) {
				if (mItemsCanFocus && hasFocus() && !selected.hasFocus()) {
					final boolean focusWasTaken = (selected == focusLayoutRestoreDirectChild && focusLayoutRestoreView != null && focusLayoutRestoreView.requestFocus()) || selected.requestFocus();
	 ...
	 */
	private int measureWidthOfChildren(int heightMeasureSpec, int startPosition, int endPosition, final int maxWidth, int disallowPartialChildPosition) {
	...
	private void setupChild(View child, int position, int top, int left, boolean flow, boolean selected, boolean recycled) {
		...
		final boolean isPressed = touchMode > TOUCH_MODE_DOWN && touchMode < TOUCH_MODE_DRAGGING && mMotionPosition == position;
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "TwoWayView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " selectedId=" + selectedId + " firstId=" + firstId + " viewStart=" + viewStart + " height="
					+ height + " position=" + position + " checkState=" + checkState + "}";
		}
