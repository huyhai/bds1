...
public class CalendarCard extends RelativeLayout {
	...
	private void init(Context ctx) {
		...
		final View layout = LayoutInflater.from(ctx).inflate(R.layout.card_view, null, false);
		...
		cardDays = (LinearLayout)layout.findViewById(R.id.cardDays);
		...
		addView(layout);
		cardDays.post(new Runnable() {
			
			@Override
			public void run() {
				HotdealApp.height1=cardDays.getHeight();
				
			}
		});
		cardTitle.post(new Runnable() {
			
			@Override
			public void run() {
				HotdealApp.height2=cardTitle.getHeight();
